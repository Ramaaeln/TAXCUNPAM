...
export default function CreateQuiz() {
  ...
  const [form, setForm] = useState({
    title: "",
    description: "",
    duration_minutes: 60, // Default disesuaikan ke 60 menit yang lebih umum
    start_time: "",
    end_time: "",
  });
  const [startDate, setStartDate] = useState(null);

  // Kalkulasi Waktu Selesai berdasarkan Durasi
 ...
 async function handleSubmit(e) {
  ...
  if (form.duration_minutes <= 0) {
    setMessageType("error");
    setMessage("Durasi kuis harus lebih dari 0 menit.");
    return;
  }
  ...
  if (!token) {
    setMessageType("error");
    setMessage("Sesi admin telah berakhir. Silakan login kembali.");
    return;
  }
  ...
  try {
    ...
    // 3. Jika Berhasil, Tampilkan Notifikasi & Reload Halaman ke Dashboard
    if (res.data?.success || res.status === 200 || res.status === 201) {
      setMessageType("success");
      setMessage("Quiz berhasil dibuat! Mengalihkan ke dashboard...");

      // Tunda 600ms agar user sempat membaca pesan sukses, lalu hard reload ke dashboard
      setTimeout(() => {
        window.location.href = "/utcbt-internal/dashboard";
      }, 600);
    } else {
      // Jika backend merespon status 200 tapi membawa flag success: false
      setLoading(false);
      setMessageType("error");
      setMessage(res.data?.message || "Gagal membuat quiz.");
    }
  } catch (err) {
    // 4. Jika Request Error (400, 401, 500, dll.), MATIKAN loading agar tombol kembali aktif
    setLoading(false);
    setMessageType("error");
    setMessage(
      err.response?.data?.message ||
        "Terjadi kesalahan saat membuat quiz. Silakan coba lagi."
    );
  }
}
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              <input
                ...
                placeholder="Contoh: Kuis Ujian Akhir Semester"
            ...
            <div>
              ...
              <div className="relative max-w-xs group">
                ...
                <input
                  ...
                  onChange={(e) => setForm({ ...form, duration_minutes: Math.max(1, Number(e.target.value)) })}
